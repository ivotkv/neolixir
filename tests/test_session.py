...
def test_get(m):
    ...
    assert m.session.get(m.engine.get_node(n1.id)) is n1
    assert m.session.get(n2._entity) is n2
    assert m.session.get(m.engine.get_node(n2.id)) is n2
    assert m.session.get(r._entity) is r
    assert m.session.get(m.engine.get_relationship(r.id)) is r
